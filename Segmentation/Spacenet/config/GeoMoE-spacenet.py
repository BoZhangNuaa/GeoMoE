############################### default runtime #################################
custom_imports = dict(
    imports=['config.OptimConstructor', 'config.GeoMoE',
             'config.EncoderDecoder_AUX', 'config.spacenetv1'],
...
data_root = '/data1/bozhang/datasets/spacenet'
crop_size = (384, 384)
train_pipeline = [
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        data_root=data_root,
        data_prefix=dict(
            img_path='train/3band', seg_map_path='train/mask'),
...
val_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        data_prefix=dict(img_path='val/3band', seg_map_path='val/mask'),
...
val_evaluator = dict(type='IoUMetric', iou_metrics=['mIoU', 'mFscore'], ignore_index=255)
...
model = dict(
    ...
    backbone=dict(
        ...
        moe_mlp_ratio=0.75,
        mlp_ratio=[4, 4, 4],
        pretrained='/data1/bozhang/ICASSP/GeoMoE.pth',
    ),
    decode_head=dict(
        type='UPerHead',
